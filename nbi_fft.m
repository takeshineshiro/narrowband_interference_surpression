   ...
   %   title('??????????????????????????????');
     ...
     % title ('???????????????????????????');
       ...
       delay_ex = [zeros(1,fft_N/2)  rev_ss(1:rev_len-fft_N/2)];           % delay fft_N/2
        ...
        for  i = 1:Max
            if(i==1)
                 black_delay  =  black_coe(fft_N/2+1:end);
            else
                 black_delay  =  [black_delay,black_coe];
            end     
            
        end
        
              % black value delay 
       zero_fill   =  zeros(1,fft_N/2);
       black_delay =  [black_delay,zero_fill];
       
      
              
       win_ss       =  rev_ex.*black_ex ;                                  % add  black window
       
       delay_win    = rev_ex.*black_delay ;                              % add black window delay
    ...
    ifft_total  =  ifft_buffer + ifft_delay_buffer ;              % just add
...
for  i  = 1:length(decode_buffer)
  ...
  for i  =1:length(bit_out)
          if (bit_out(i)-bit_gen(i))            %%%error  calculate%%%%%
